...
| Next evidence target | design and causally validate a bounded batch-preserving model/event pipeline: parallel preparation/direct inserts with strictly ordered atomic commits and no stale state/index reservation |
| Durable run register | [`model-e2e-run-registry.csv`](performance-runs/model-e2e-run-registry.csv) |

Last updated after the E78 final-source P3 verification on 2026-08-02. This table is updated whenever a run changes the accepted base, current
