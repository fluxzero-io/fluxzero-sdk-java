...
| Latest causal diagnosis | E376-E382 map the first full-route admission curve: 32 does not bind, **16 leads at 902,873/s**, 12 repeats healthily at 890,091/s, 8 makes larger result transactions and the best local writer rate but loses some insert-ahead, and 4 binds commands too and collapses. E383's max-20 limit did not bind, but captured the separate cache cliff directly: physical command reads jumped from about 60k to about **5M rows/s** while the command writer remained healthy at 1.328M/s. The fixed 65,536-entry recent-command window turns a transient cached-range miss into a persistent JDBC feedback loop; stabilize that byte-bounded cache guardrail before the profiler-free 16-versus-unbounded bracket. |
| Next evidence target | Keep the complete durable route intact and give only the command recent-message cache enough count headroom for its existing 64-MiB byte budget to remain the effective bound. Confirm that this removes the physical-read cliff, then run a profiler-free balanced unbounded-versus-16 bracket. |
