...
| Latest causal diagnosis | E80-E83: a bounded packed pipeline reached **two active model transactions**, but naive overlap fragmented transactions and arrival-gating preserved them without improving model wall time; the shared PostgreSQL write path, not lack of queued transactions, owns the serial boundary |
| Next evidence target | causally validate prewriting immutable event/model rows on independent bounded lanes while keeping them logically invisible; make one small ordered atomic visibility commit publish both heads only after every prewrite is durable |
| Durable run register | [`model-e2e-run-registry.csv`](performance-runs/model-e2e-run-registry.csv) |

Last updated after the E80-E83 bounded-pipeline rejection on 2026-08-02. This table is updated whenever a run changes the accepted base, current
