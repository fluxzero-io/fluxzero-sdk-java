...
| Latest closed diagnostic | E74 forced existing direct message-log tails on the intact profiled route: event staging fell 8.382 -> 0.001 ms and packed-model service 24.555 -> 19.431 ms, while E2E moved 263,353 -> 269,039/s; the mechanism is real but requires a narrow production candidate and canonical proof |
...
| Next evidence target | make only sufficiently large co-located model/event transactions write their underfilled tail directly, preserving low-rate staging and atomicity; then run matched canonical E2E before proceeding to the batch-preserving parallel-insert/ordered-commit pipeline |
| Durable run register | [`model-e2e-run-registry.csv`](performance-runs/model-e2e-run-registry.csv) |

Last updated after the E74 direct-tail causal profile on 2026-08-02. This table is updated whenever a run changes the accepted base, current
