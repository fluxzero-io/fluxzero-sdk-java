...
| Latest causal diagnosis | E92 reranked accepted P3 with simultaneous SDK/Runtime CPU profiles. Runtime compression is 13.99% inclusive, but the overlapping message-store/tracking/WebSocket clusters are 19.42%/21.62%/24.73% and 24.77% of Runtime samples are waiting. SDK work is similarly distributed. Combined with E75's serial 20.268-ms model/event transaction, this rejects a new Java micro-hotspot as the route explanation: the largest intact-route constraint remains the ordered physical durability boundary. |
| Next evidence target | remove one of the duplicate global-event/model-stream physical writes, or make both representations share one durable body, inside one accepted P3 transaction; do not add another transaction, weaken visibility, shrink durable batches or add a queue/timer |
| Durable run register | [`model-e2e-run-registry.csv`](performance-runs/model-e2e-run-registry.csv) |

Last updated after the E92 accepted-P3 full-route CPU rerank on 2026-08-02. This table is updated whenever a run changes the accepted base, current
...
17. Causally validate the largest canonical constraint by narrowly relieving or accelerating it while retaining the
...
18. Confirm each positive candidate through matched non-JFR runs. Checkpoint every statistically convincing, correct
