...
| Latest causal diagnosis | E79: **359/365 (98.4%)** sampled commands were resolved by a later client poll and only **6/365 (1.6%)** by the storage notification; C06's 43.147 ms is consumer-backpressure residence, not tracking service (scan mean 0.682 ms) |
| Next evidence target | design and causally validate a bounded batch-preserving model/event pipeline: parallel preparation/direct inserts with strictly ordered atomic commits and no stale state/index reservation; measure whether C06 falls as downstream capacity rises |
| Durable run register | [`model-e2e-run-registry.csv`](performance-runs/model-e2e-run-registry.csv) |

Last updated after the E79 tracking-resolution diagnosis on 2026-08-02. This table is updated whenever a run changes the accepted base, current
