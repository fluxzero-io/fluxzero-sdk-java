...
| Latest causal diagnosis | E376-E382 map the first full-route admission curve: 32 does not bind, **16 leads at 902,873/s**, 12 repeats healthily at 890,091/s, 8 makes larger result transactions and the best local writer rate but loses some insert-ahead, and 4 binds commands too and collapses. E383's max-20 limit did not bind, but captured the separate cache cliff directly: physical command reads jumped to about **5M rows/s** while the command writer remained healthy at 1.328M/s. E384 leaves the complete route intact and makes the existing 64-MiB byte budget effective instead of the 65,536 count cap: the command cache naturally holds about 308k entries, physical reads fall 97.9%, every observed fallback is the normal `at-tail` race, and E2E returns to 881,840/s. |
| Next evidence target | Run a profiler-free balanced unbounded-versus-16 bracket with the same byte-bounded command-cache guardrail on both sides, waiting for Docker/virtualization writeback between invocations. Separately harden the default cache count/weight policy across tiny and large payloads before accepting it as production behavior. |
