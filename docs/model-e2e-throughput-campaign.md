...
5. Run E60 as a same-source compression-algorithm diagnostic with exact checks and wire-byte/resource accounting. If it
   is causally positive, design a compatibility-safe negotiated/adaptive compression candidate; if neutral, close that
   branch and design a structural combined commit/result pipeline rather than revisiting isolated transaction tweaks.
6. Confirm each positive candidate through matched non-JFR runs, checkpoint only a significant correct result, rerank
