...
| Best non-qualifying ceiling | E386 reaches **964,390/s** on the complete durable no-model command/result route with exact full-envelope byte accounting; its matched E389 repeat is 963,072/s. This is a stable near-1M pin, not yet the required result well above 1M/s. |
| Latest accepted checkpoint | SDK `bb8e1db2` makes every message byte budget count the exact complete native envelope without materializing lazy payload or metadata; it includes Backlog checkpoint `5f4d2c76`. Runtime performance base remains P3. |
| Current production code | accepted P3 Runtime plus SDK `bb8e1db2`; the Runtime message-store remains intentionally unbounded and at its legacy 4,096-message backlog for the no-model pin. The opt-in in-flight property remains diagnostic; 16 is rejected as a default for this route. |
| Latest causal diagnosis | E397 resets PostgreSQL statistics only after warm-up and observes 16,864 commits but 9,961 WAL syncs during the intact 10.49M-command/result measurement: **1.69 commits per sync**. WAL write averages only **0.050 ms** and physical `fdatasync` **0.670 ms**, versus the 2.484-ms Java result-commit interval in E390. The remaining time is therefore server commit coordination/follower wait/JDBC roundtrip rather than raw WAL writing alone; these overlapping quantities cannot be subtracted without wait-event evidence. |
| Next evidence target | Sample PostgreSQL wait events during the unchanged measured phase and classify active `COMMIT`, insert, position update and read backends. Optimize only the proven limiting mechanism, then require profiler-free no-model throughput stably and materially above 1M/s before returning to the full model route. |
| Durable run register | [`model-e2e-run-registry.csv`](performance-runs/model-e2e-run-registry.csv) |

Last updated after E397 on 2026-08-02. This table is updated whenever a run changes the accepted base, current
