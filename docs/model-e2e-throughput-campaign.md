...
| Latest closed diagnostic | E73 proved that the 41.621-ms command `strategy update -> batch resolved` interval is predominantly demand residence while an SDK tracker processes its preceding batch, not Runtime scheduling or scan service; [`model-e2e-p2-route-anatomy.md`](performance-runs/model-e2e-p2-route-anatomy.md) contains the atomic distributions and correction |
| Current production code | accepted P2 behavior plus low-allocation JFR-only E73 observability in both repositories; no E62–E68 production candidate code remains |
| Next evidence target | causally validate a batch-preserving parallel-preparation/ordered-commit model-store pipeline without repeating E42 fragmentation; retain natural transaction shape and measure complete E2E throughput, queue bytes, overlap and publication order |
| Durable run register | [`model-e2e-run-registry.csv`](performance-runs/model-e2e-run-registry.csv) |

Last updated after the E73 command-tracking demand/scan split on 2026-08-02. This table is updated whenever a run changes the accepted base, current
