...
The Phase 14 paired-tree table used the benchmark's `ZIPF` selector. With 48 leaves, concurrency 32 and 2,000 actions,
...
The JDBC model-action backlog previously had an all-or-nothing optimistic batch decision. One later action that read a
model written by an earlier action made every independent action in that intake batch fall back to individual,
sequential JDBC commits.

The runtime now divides consecutive `ACCEPT` actions into ordered conflict-free waves:

- an action that reads a model written earlier moves behind that write;
- independent actions stay in the earliest batchable wave;
- write order for the same target remains stable, including write-only targets;
- `FAIL` and `RETRY` actions remain individual ordering barriers;
...
relationships, documents and action idempotency retain their existing contracts.
...
Default `ACCEPT` model actions whose read sets overlap are now locally ordered. A waiter re-evaluates assertions,
interceptors and applies against the cache updated by its predecessor before committing. Disjoint model IDs remain
fully parallel, and multi-model acquisition is atomic for the complete read set, so local actions cannot deadlock.
Strict `FAIL`/`RETRY` actions retain their existing runtime-observable conflict path.
...
### Conflict-free 10,000-action baseline

| metric | aggregate | independent models | model/aggregate |
| --- | ---: | ---: | ---: |
| event-only throughput | 1,089.0 actions/s | 1,493.9 actions/s | **1.372×** |
...
A separate 2,000-action direct-search run measured 1,093.6 aggregate versus 1,153.3 model actions/s
(`1.055×`). Direct model search was 0.399/0.856/5.186 ms p50/p95/p99, versus
0.577/1.668/18.900 ms for the aggregate document.

### Skewed 10,000-action contention profile
...
| metric | aggregate | independent models | model/aggregate |
| --- | ---: | ---: | ---: |
| event-only throughput | 1,059.8 actions/s | 818.7 actions/s | **0.773×** |
...
Before local coordination, the same short 2,000-action event-only profile measured 300.3 model actions/s versus
1,132.9 aggregate actions/s (`0.265×`). Conflict-free runtime waves alone raised the model result to 374.1 actions/s.
With both changes it reached 842.1 actions/s versus 1,077.1 (`0.782×`). The longer result above confirms that this is
...
2,000-action run measured 553.6 model versus 949.9 aggregate actions/s (`0.583×`). Removing that last gap would require
...
- The focused JDBC model-action store suite passed 78 tests.
...
direct-document visibility, action idempotency and exactly-once global publication.
