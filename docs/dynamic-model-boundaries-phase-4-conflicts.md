...
Fluxzero keeps its normal single-writer-friendly behavior: stale model commits are accepted unless an application
explicitly selects a rejecting policy.

The commit carries one global `readStateIndex`. It does not carry a version vector. Every written model is already
required to occur in `readModelIds`, so that one commit scope covers both read-only dependencies and write targets.
...
- `FAIL` rejects the complete commit when any commit-scoped model head has a `lastStateIndex` greater than the commit's
  `readStateIndex`.
- `RETRY_IF_RELATIONS_UNCHANGED` rejects on the same head condition and additionally determines whether another SDK
  evaluation is safe. Retry is allowed only when no relationship touching an commit-scoped identity changed after the
  read boundary.
...
boundary. Rejected results are not stored under the commit idempotency key.
...
column or per-commit relation record.
...
`RETRY_IF_RELATIONS_UNCHANGED` loads them for the complete commit scope.
...
For co-located JDBC event and model stores, each strict commit is isolated:
...
Accepted `ACCEPT` commits remain on the original asynchronous, batched path. Strict commits intentionally pay for an
...
event append: the already-published commit must finish its model write and its global event remains single-copy. The
...
- fail with the default `ModelCommitConflictException`;
...
exhausted. The committer obtains a fresh `CommitEvaluation` from a reload supplier and reuses the rejected commit ID,
which is safe because conflicts are not retained. The supplier is the integration seam for the pinned loader in
Slice 5.1.

A rejected commit never mutates direct search documents. Only an accepted commit proceeds to the synchronous direct
...
- default stale acceptance, `FAIL`, bounded retry, custom error mapping, and non-retention of rejected commit IDs;
...
|---|---:|---:|---:|---:|
| Explicit `ACCEPT`, 5,000 × one 1-KiB target | 5,551 commits/s | 88.8 ms | 2.15× | 2.87× |
| Omitted legacy policy, same profile | 5,542 commits/s | 90.7 ms | 2.15× | 2.90× |
| Strict accepted `FAIL`, 1,000 × one 1-KiB target | 642 commits/s | 430.0 ms | 1.78× | 4.92× |

The explicit and omitted `ACCEPT` runs differ by 0.2%, within local run variance, and execute the same SQL path.
No query, row, physical-storage, or WAL structure was added to accepted default commits. The strict result quantifies
the deliberate cost of per-commit isolation and conflict eligibility; it is not a target for the normal Fluxzero path.
