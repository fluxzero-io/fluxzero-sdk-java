...
- Put commands, queries, and typed IDs in an `api` package; put aggregates, entities, and value objects in `api.model`; keep handlers and endpoints near the domain package.
- Keep aggregate and entity state immutable. Use Java records for payloads and value objects where possible.
