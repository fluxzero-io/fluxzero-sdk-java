...
| `loadCurrentGraph(id)` / `graph.current()` | Lazy relationship Graph | In a mutation: same attempt and readset, not a newer snapshot. Otherwise: fresh storage boundary pinned during the call, not the old event's boundary. Values remain lazy and authoritative, not a no-replay API |
