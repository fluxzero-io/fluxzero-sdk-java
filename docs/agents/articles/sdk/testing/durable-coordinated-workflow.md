...
`StartAssetJob` must only apply `RecordAssetJob`; it must not schedule or send a request immediately after `assertAndApply(...).get()`. `AssetJobAcceptedEffects` is a tracked `@HandleEvent` consumer that reloads the committed Model, creates the stable deadline, and publishes the requests. This ordering makes new aliases visible before a processor can answer. Read Model commit and effect boundaries before implementing this slice.
...
Drive the public query and a real secondary-reference message through separate phases. The resolver must publish the durable primary-ID command; that command, not the resolver, mutates Model state:
...
## Combine Model, correlation, and active-schedule reconstruction

Continuing with `andThen()` does not prove reconstruction. Create a new default fixture and seed only the recorded
artifacts that production would treat as durable: Model events plus the still-active command schedule. Then use a
...
TestFixture reconstructed = TestFixture.create(
        ...
        .withProperty("processor.caption.base-url", "https://caption.test/api")
        .withProperty("processor.artwork.base-url", "https://artwork.test/api")
        .givenModelEvents(assetJobId, recordedStart, recordedArtworkConfirmation)
...
`givenModelEvents(...)` reconstructs Model aliases from supplied history, while `givenScheduledCommands(...)`
