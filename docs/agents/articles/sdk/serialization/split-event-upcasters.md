...
For Models, start with the Model example in the reconstruction article: register casters, then call
`givenModelEvents(id, oldSerializedEvent, ...)`. It also decodes input as an event stream before applying current
events. The Model examples below remain for existing Model workflows. Neither Given route preserves an
old writer's stored bytes; the Model migration article describes the retained-store test for that stronger claim.

For this TestFixture setup, `givenModelEvents(..., SerializedMessage)` and `givenEvents(SerializedMessage)` deserialize the input as an event stream and flatten every result from a one-to-many upcaster.
...
through the registered caster chain and reconstructs the Model. Manually calling `serializer().deserialize(...)`
...
`SerializedMessage` to `givenModelEvents(...)`. The fixture deserializes it as `MessageType.EVENT`, runs the registered
...
## Prove Model reconstruction for one or many results

`givenModelEvents` applies every split result to the selected Model. Query or load the reconstructed Model after the Given phase and assert the combined state:

```java
TestFixture.create(AssetJob.class)
        .registerCasters(new LegacyAssetJobUpcaster())
        .givenModelEvents(new AssetJobId("asset-job-42"), legacyAssetJobCreated())
...
order, target the Model, and combine into the expected current state.
