...
// The Model already exists. Calculate a replacement against this exact revision:
Graph<Counter> observed = Fluxzero.loadCurrentGraph("counter", Counter.class);
boolean replaced = observed.compareAndSet(new Counter("counter", observed.get().value() + 1));

// Or calculate again on a fresh transactional view after a rejected commit:
Graph<Counter> after = observed.updateAndGet(
        current -> current.update(value -> new Counter(value.id(), value.value() + 1)), 2);
```
...
```java
Graph<Counter> consumed = Fluxzero.loadCurrentGraph("counter", Counter.class)
        .getAndUpdate(Graph::delete);
Counter released = consumed.get(); // deletion has committed before this line
```
