...
Start with plain `@Model`; keep optional storage and projection settings out of ordinary state declarations.
Only introduce them when a query or other concrete requirement needs them. Read `/docs/sdk/models/configuration`
for those options, and `/docs/sdk/entities/graph-search` when choosing a query.
Use independent Models with `@Parent` for relationships; the developer relationship guide covers the specialized
embedded-member case.
