...
Event metadata remains opaque to the runtime. The SDK-authored model action ID and substep ordinal continue to resolve
historical model boundaries, but the complete target-bearing commit receipt is no longer permanent storage.

The durable action core retains:

- the action ID as a durable idempotency fence;
...
For new actions, the core does not retain raw target IDs. A duplicate request already carries those IDs; the runtime
combines them with the durable positions to reproduce the exact accepted result. Existing full action rows remain
...
the SDK-authored action ID plus substep ordinal and resolves their `stateIndex` from the durable action core.

## Defaults

The default transient receipt retention is one hour. It can be overridden with the positive ISO-8601 duration system
property `fluxzero.modelActionReceiptRetention`. Because expiry occurs at complete hourly partition boundaries,
...
old partitions may be removed at restart, but a tracker already at the last committed action must not receive a reset
whose update position is newer than the namespace itself.

## Storage and hot-path shape

- The durable action core remains hash-partitioned by the stable Fluxzero action segment. Existing point lookup,
...
- Temporary receipts use one current time partition and one ordered primary index. Tracking therefore scans a
  contiguous `stateIndex` range instead of merging the former 32 action partitions.
- Targets with a stored model event reuse their durable stream membership for erasure lookup. The uncommon
  ...
  dropped with the receipt rather than accumulating in the legacy permanent action-target index.
- Creating and purging partitions is outside the action transaction. The action transaction then writes core,
  receipt, streams, heads, relationships and publication as one package.
- Retention is never checked per action. It runs once at namespace activation and only when an action crosses into a
...
The complete `JdbcModelActionStoreTest` passed 80 tests after the storage split. Focused contracts additionally prove:
...
- hard delete still returns erased target identities for an old action retry and sanitizes any live receipt;
...
| --- | ---: | ---: | --- |
| 20,000 published one-target actions, run 1 | 4,397 actions/s | 4,975 actions/s | run variance, no observed regression |
| same profile, run 2 | 4,917 actions/s | 4,852 actions/s | -1.3% |
...
| 5,000 published ten-target actions | 1,674 actions/s | 1,655 actions/s | -1.1% |
...
One instrumented 20,000-action run attributed 0.44 MiB to durable compact results and 0.81 MiB to temporary receipts.
...
in-flight action just before the accepted command result seeded the cache and start an unnecessary suffix load. Local
actions now install a target-scoped fence before commit. The accepted result clears the fence after updating the cache;
