...
`CompressionAlgorithm.LZ4` is no longer part of the Java/Kotlin API. Replace explicit uses with `ZSTD`.
ZSTD retains Fluxzero compression wire ID 2; ID 1 is retired. The document MessagePack content and format identifier
remain unchanged. The Runtime retains legacy document decoding and returns size-prefixed LZ4 to clients that do not
request ZSTD documents. Applications reading old blobs directly, outside the Runtime, must still convert them before
passing them to SDK 2.0. This is an intentional SDK API break; it does not require rewriting existing Runtime storage.

Storage integrations can retain an original payload while supplying an internal lazy decoder with
`SerializedDocument.withDocumentDecoder(...)`. `DefaultDocumentSerializer.deserializeUncompressed(...)` parses the
versioned document content after a storage owner has decoded its envelope. Neither API adds an LZ4 codec to the SDK.
The Runtime uses these boundaries to forward LZ4 to LZ4 clients and ZSTD to ZSTD clients without recompression.
