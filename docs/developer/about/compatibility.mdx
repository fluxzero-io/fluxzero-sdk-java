...
Clients without a recognized document capability header receive size-prefixed LZ4. Explicitly advertised codecs
...
supply this new negotiation behavior merely because a newer producer is connected. With no capability header,
Runtime conservatively assumes LZ4-only support, even if that older SDK also happens to understand ZSTD.
