...
Such an SDK needs the new Runtime conversion when reading documents written by SDK 2.0. An older Runtime cannot
supply this new negotiation behavior merely because a newer producer is connected. Explicit document capabilities take precedence. Without them, Runtime honors a document-specific ZSTD request;
otherwise, advertised or selected ZSTD transport identifies older Java SDKs that already read Fluxzero-framed
LZ4, ZSTD and NONE documents. Those SDKs retain supported stored envelopes. A raw ZSTD frame receives only the
Fluxzero framing header needed by that older document reader; it is not recompressed. Only without this support
information does Runtime fall back to size-prefixed LZ4.
