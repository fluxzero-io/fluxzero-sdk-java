---
title: Installation
description: Start building with a coding agent, use the Fluxzero CLI, or install the SDK yourself.
sidebar:
   order: 20
...
<span id="start-with-a-coding-agent-recommended"></span>

## Start with a coding agent <Badge text="Recommended" class="heading-badge" />

The easiest way to build with Fluxzero is to open [Get started](/get-started) and copy the instruction into your coding agent. The agent will connect to the current Fluxzero guidance and can create, run, and evolve the application with you.
...
If you prefer to work directly in the code, use the Fluxzero CLI to create a project, or add the SDK to an existing project with Maven or Gradle as described below.

## Using the Fluxzero CLI

The **Fluxzero CLI** is the most convenient way to scaffold, build, and run Fluxzero applications.
The CLI itself is a native executable and does not require Java. Projects using Fluxzero SDK 2.x require Java 25 or newer.
...
1. Get the Fluxzero CLI
   ...
   <Tabs>
      ...
      <TabItem label="Manually">
         ...
         and run it with Java 25 or newer when using SDK 2.x:
...
## Choose the application packages

Before replacing starter behavior with product code, choose the owning domains and concrete package paths using
[Application package structure](/docs/getting-started/package-structure). It includes the Java/Kotlin placement rules,
a two-domain example tree and the checks to apply before finishing.
