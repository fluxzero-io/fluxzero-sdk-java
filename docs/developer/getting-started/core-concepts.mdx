---
title: Core concepts
description: How Fluxzero keeps your product behavior visible in clean code while the cloud handles the foundation.
sidebar:
    order: 11
---
...
Either way, the application contains the heart of your product: the messages, logic, and state.
...
- **Recommended:** avoid putting the sending user ID in command payloads.
Inject `Sender` in `@Apply`, `@AssertLegal`, or handler methods when you need caller context.

- **Keep `@Apply` pure and deterministic.**
This method may be re-run during [event sourcing](/docs/guides/modeling-and-persistence/updating-entities) — avoid randomness, timestamps, or I/O actions here.
...
<Aside type="note">
    Note that we use Spring `@Component` annotation to register the handler.
    In previous examples, such as with queries, we didn't need to register anything manually because the query class itself contained the handler, allowing Fluxzero to [automatically discover it](/docs/guides/configuration/configuring-fluxzero/#spring-auto-configuration).

    In this case, the handler is a separate class, so we need to explicitly make it discoverable.

    Fluxzero will automatically scan all your Spring Beans for handlers.
    However, if you're not using Spring, you can also register handlers manually using the `Fluxzero.registerHandler(...)` method.
    See [Configuring Fluxzero](/docs/guides/configuration/configuring-fluxzero/) for more details.
</Aside>
...
See the [messaging handling guide](/docs/guides/messaging/message-handling) for more advanced usage.
...
For deeper control — like async behavior, parallelism, error handling, and more — see the [messaging handling guide](/docs/guides/messaging/message-handling).
...
For more advanced background on HTTP endpoints in Fluxzero, including handling websocket connections, having multiple handlers for the same request, and so on, see [HTTP endpoints](/docs/guides/messaging/handling-web-requests).
...
When disabled, the entity will be persisted as a snapshot instead of a list of events. See [event sourcing](/docs/guides/modeling-and-persistence/entity-loading) for more.
...
<Tabs>
  ...
  ```kotlin
  @Aggregate
  data class Project(val id: ProjectId, @Member val tasks: List<Task>) { /* ... */ }
