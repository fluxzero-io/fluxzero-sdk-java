...
`@Revision`s) for each affected top-level type. A materialized Model Graph is the deliberate exception: every root and
descendant is recorded as a separate serialized node and is upcast independently.
</Aside>
...
To modify the **type name**, or the type and content together, return a `Data<ObjectNode>`. This ordinary upcaster is
the preferred owner of a class rename; a separate typecaster is not required:
...
<Tabs>
  ...
  <TabItem label="Kotlin">
    ...
    fun renameType(data: Data<ObjectNode>): Data<ObjectNode> {
        data.value.set<JsonNode>("userId", data.value.remove("id"))
        return data.withType("com.example.RegisterUser")
    }
    ```
  </TabItem>
...
A materialized Graph has no single schema revision. Its hidden manifest records the serialized type and revision for
every root and descendant placement. When a node is first accessed, Fluxzero passes that node through the same
registered serializer chain as its direct Model document or snapshot.

For every changed Model type:

1. increment that type's `@Revision`;
2. register an ordinary `@Upcast` for its serialized source type and revision;
3. return `Data<ObjectNode>` when the upcast also changes the type name;
4. test the old node JSON with `TestFixture.whenUpcasting(...)`.

No Graph-specific upcaster is needed. Lazy upcasting makes reads correct immediately. If search should also retain the
evolved JSON, use a projection-only `@HandleDocument(modelGraph = RootModel.class)` handler as described in
[Tracking, updating documents](/docs/guides/modeling-and-persistence/tracking-and-updating-documents#evolving-a-materialized-model-graph).
Returning its complete Graph conditionally replaces only the derived materialized document; it never rewrites Model
events, snapshots, direct documents or relationships.
...
In standalone tests, register casters before the first conversion or historical Given step:

```java
TestFixture.create().registerCasters(new CreateUserUpcaster())
        .whenUpcasting("/users/create-user-rev0.json")
        .expectResult(expectedCurrentCommand);
```

The resource's type and revision must match that caster; `expectedCurrentCommand` must retain the old ID and business
values. `TestFixture.create(caster)` registers a handler, not a caster. Kotlin uses
`TestFixture.create().registerCasters(CreateUserUpcaster()).whenUpcasting<CreateUser>(resource)`.
For a complete value-preserving `name` → `details.name` example, Model event reconstruction and an old-writer/new-reader
storage test, use [Testing Model migrations](../../modeling-and-persistence/model-migration-tests).
...
- Use `ObjectNode` for structural changes, or `Data<ObjectNode>` when serialized content, type or revision evolve
