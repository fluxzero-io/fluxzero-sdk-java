...
to change. You can register multiple exact aliases and package aliases together:
...
The same configuration can be supplied as a comma-, semicolon-, or newline-separated application property. Add `.*`
on both sides to identify a package alias:
...
resulting type before deserialization. Configure them on `FluxzeroBuilder`, through
`fluxzero.serialization.typeAliases`, or directly on the fixture with `registerTypeAlias(...)` and
`registerPackageAlias(...)`. A property named `revision` without the `@` prefix is always ordinary payload data.
