...
## Asynchronous handler results

For automatic handler-result filtering, a `CompletableFuture` is filtered after successful completion, using
the original request context and viewer even when completion happens on another thread. Failed or cancelled
futures propagate without running content filters. This applies equally to ordinary values and Graph results.
...
```
  ...
  <TabItem label="Kotlin">
```kotlin
    data class GetOrder(val orderId: OrderId) : Request<Order?> {
        @HandleQuery
        @FilterContent
        fun handle(): Order? {
            return Fluxzero.getDocument(orderId, "orders")
