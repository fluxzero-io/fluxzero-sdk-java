...
An `@LocalOnly` command or query is the fail-closed exception: it requires exactly one result-producing local handler.
Invalid selection throws `LocalOnlyDispatchException` before monitoring, serialization, or forwarding, including for
async and bulk calls.
...
graph TD
    ...
    HANDLERS -->|Invalid @LocalOnly selection| FAIL_LOCAL[Throw LocalOnlyDispatchException]
