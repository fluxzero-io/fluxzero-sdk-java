...
SDK-controlled Model retries and reevaluation use named virtual workers; bulk serialization, packed Model-event
decoding and multi-Model replay use an SDK-owned CPU pool. Existing ordering, batching and admission limits still
apply. These execution paths do not schedule work on the JVM common pool. This is not a promise about application
executors or JDK internals: native HTTP/WebSocket transport completion can still depend on the JDK's internal pool.
...
commands in the same package can therefore share a consumer without a marker interface. The generated consumer uses
the default consumer configuration for the message type as its template.
...
its own generated consumer. With `defaultAppConsumer`, it joins the shared default consumer. A matching builder
configuration is used before the fallback, while an explicit `@Consumer` remains most specific.
...
An absolute HTTP URL does not select a separate consumer. In tests, give blocking external-service stubs
an explicit consumer while preserving application consumer configuration; see
[HTTP stubs and consumers](/docs/guides/messaging/testing-web-endpoints/#external-http-stubs-and-consumers).
Same-name incompatible configurations report the handler types in that registration without invoking their
filters again. Identical configurations can accept additional handlers after startup; this neither reconfigures
the live consumer nor rewinds its position.
...
| `awaitSendAndForgetFutures` | If true, waits for outgoing command completions before storing position. | `true`                    |
