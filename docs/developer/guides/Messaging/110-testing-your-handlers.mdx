...
description: Testing your handlers
slug: docs/guides/messaging/testing-your-handlers
sidebar:
  order: 110
---
...
Registered exact and package type aliases apply to `@class`, so existing fixtures can retain a legacy fully qualified
name after a class or package move. Configure aliases on `FluxzeroBuilder`, with
`fluxzero.serialization.typeAliases`, or directly on the fixture using `registerTypeAlias(...)` and
