...
Use `User.id()` for ownership checks, audit references, and other behavior that needs a stable user identity. The SDK
...
a distinct principal/display name, not for identity comparisons.
...
accept complete-user metadata. If an application's stable ID changes, its provider must explicitly resolve
