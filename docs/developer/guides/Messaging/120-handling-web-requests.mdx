...
A handler may return a `Graph<T>` directly, or use one as a `WebResponse` payload. The transported body is its
composed JSON document, including selected child paths and `@GraphProperty` values—not a live Graph that the
receiver can mutate. SDK callers can read it with `response.getPayloadAs(JsonNode.class)`. Enable
`@FilterContent` on the handler when the Graph needs viewer-specific filtering; serialization does not enable
filtering implicitly. Ordinary typed response payloads keep their existing deserialization contract.
...
Path parameters are percent-decoded once as UTF-8 after route matching and before type conversion.
For example, `order%3A1` becomes `order:1`; a literal `+` stays `+`. Encoded slashes (`%2F`)
belong to the parameter value and do not introduce route segments. Do not decode the injected value again.
