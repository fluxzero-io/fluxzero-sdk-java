...
For deletion-only cleanup, [parent-owned schedules](./086-parent-owned-schedules.mdx) offer a simpler contract:
`@Parent` on the scheduled payload cancels it when that committed Model is deleted, including an applicable
`@Apply` returning `null` or a cascade. Reconciliation below remains useful for status and deadline changes,
and delivered commands still need guards.
