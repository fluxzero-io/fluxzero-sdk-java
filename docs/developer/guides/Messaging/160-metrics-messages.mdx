...
| Metric message | Description |
|----------------|-------------|
| **CommitModels** | Commits model events, documents, snapshots and relationship deltas as one package. |
...
| **GetModelChange / GetModelChangeResult$Metric** | Resolves the target count and exact boundary of one durable model-commit substep for a graph-change handler. |
...
| Metric message | Description |
|----------------|--------------|
| **AppendEvents** | Appends events to a legacy aggregate stream. |
| **GetEvents** | Requests events from a legacy aggregate stream. |
| **GetEventsResult$Metric** | Reports metrics for `GetEvents`. |
| **DeleteEvents** | Deletes a legacy aggregate stream. |
