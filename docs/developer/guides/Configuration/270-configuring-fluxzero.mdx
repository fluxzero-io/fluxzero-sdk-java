...
The aliases `compatibility` and `hybrid` are accepted for the same mode. For SDK validation and migration tests,
`fluxzero.metadata.mode=strict-generated-only` additionally rejects JVM backend categories that are still tracked as
migration debt.
