...
- `addTypeAlias(...)` and `addTypeAliases(...)` map one or more legacy serialized type names to current type names.
- `addPackageAlias(...)` and `addPackageAliases(...)` migrate a package prefix and all its subpackages while preserving
  the remaining class-name suffix. The same aliases can be configured with
  `fluxzero.serialization.typeAliases`; programmatic aliases take precedence for the same source.
