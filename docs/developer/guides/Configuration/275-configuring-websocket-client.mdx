...
by default. Explicit completion concurrency still wins, so an application can opt into 32 after validating its
workload. Using virtual threads does not remove the configured resource limits.
