...
jitter over capped exponential ceilings of 1, 2, 4, 8, 16, then 30 seconds. A successful connection resets the retry
...
use the existing lazily populated fixed platform-thread pool. Both routes default to eight. Explicit completion
concurrency still wins on both routes, so a Java 25 application can opt into 32 after validating its workload.
