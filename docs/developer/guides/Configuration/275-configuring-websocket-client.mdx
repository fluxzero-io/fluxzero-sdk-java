...
| `maxConcurrentRuntimeResultCompletions`      | Admitted groups and result completions/continuations per client                  | `32` on Java 25+, `8` on Java 21–24      |
...
and actively completes at most 32 message groups/results on Java 25 and newer or eight on Java 21 through 24 by
default. Customer callbacks never run on a decode worker. This keeps pong and decode progress independent from slow
...
The executor policy itself does not change: Java 25 and newer use virtual completion workers with a default concurrency
of 32, while Java 21 through 24 use the existing lazily populated fixed platform-thread pool with a safe default of
eight. Explicit completion concurrency still wins on both routes.
