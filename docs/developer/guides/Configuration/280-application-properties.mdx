...
```properties
fluxzero.defaults.version=2026.09.09
```

This enables the package-scoped consumer default, the newer reconnect policy and all earlier versioned defaults. To
choose one behavior explicitly without changing the defaults version, set the dedicated property directly. Existing
applications that omit `fluxzero.defaults.version` retain the original shared application consumer, immediate implicit
periodic start and fixed one-second WebSocket reconnect interval.
