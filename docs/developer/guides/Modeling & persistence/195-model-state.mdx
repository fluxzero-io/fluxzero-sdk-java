...
For polymorphic members, put handler contracts on the declared member type or its sealed permitted subtypes.
Subtype-only handlers in an open hierarchy are not automatically discoverable during Model planning; declare the
shared handler contract instead.
