...
Creating the graph does not load the complete tree. It starts with the same direct model load as value injection and
fetches relationship state only when navigation is requested. Typed ancestor navigation follows persisted relationship
identities and materializes only the selected ancestor value, so `graph.ancestorModel(Organisation.class)` need not
load intervening location or connection values.
...
The root value is loaded immediately; the runtime pins one `stateIndex` and reconstructs related nodes in batches on
the first traversal. Every returned child is itself a graph with its own parent, root and history operations. Use
...
`@ParentId(path = "...")` edges. Pathless relationships remain available through typed `children(...)` and
