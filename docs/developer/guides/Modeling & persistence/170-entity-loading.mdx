...
description: Load independent models and explore complete model graphs
sidebar:
   order: 170
---
...
- `previous()` returns the preceding revision; durable historical values require `EVENT_SOURCED`, not `DOCUMENT` alone;
- `current()` opens the same Model on a new current boundary without modifying the historical Graph;
...
Outside historical event handling, a new detached Graph establishes its snapshot against storage, not the age of
the cached root. Reading `get()` before `children(...)` does not hide already committed relationship changes.
Typed lazy loads pin on their first storage read; untyped loads pin when resolving the root identity. Once pinned,
a Graph remains a snapshot, not a live view of later commits.
...
always resolved before an equal alias. Use parent-scoped primary identities when a child ID is meaningful only within its parent.
...
See [Model recipes](./197-model-recipes.mdx) for examples, including direct companion loads by parent ID and Model type.
...
Background cache replay runs in that repository's configured application context, not the context of its first caller.
...
record Country(@EntityId CountryId countryId, CountryDetails details) {
}
```
...
An ordinary event without a Model-commit boundary resolves directly addressed Models at one current pinned boundary. Complete graph-change subscriptions still require durable Model commit
...
what event and notification handlers need. Use `loadCurrentGraph(...)` when current intent is deliberately required:
after a synchronous nested command, or in a scheduling reconciler deciding which deadlines still belong despite
receiving an old event. For example:
...
one committed substep. `graph.previous()` is the graph immediately before that change, independent of model cache
depth when its inspected nodes have event-sourced history.
...
Cascaded logical deletions also notify the deleted child's sole-`Graph<T>` handler through the original domain event.
It receives an empty current graph and the previous value; no extra public technical event or parent-specific cleanup
handler is needed. Cascades retain their own internal change boundary. A child updated and then cascaded in one
commit can therefore produce both observations; surviving shared ancestors observe removal as well. An already
deleted ancestor is not notified twice for that deletion. The cascaded child's `previous()` retains the state before its own deletion substep: its value is available, but
an ancestor already deleted in an earlier substep is no longer reachable. It does not rewind the whole commit.
Suppressed publication and physical erasure do not manufacture domain notifications. Normal delivery retry rules apply.

**Want to use `previous()`? Keep `EVENT_SOURCED` enabled.** `DOCUMENT` alone stores current state, not historical
document versions. Neither a before-boundary nor a larger cache can reconstruct an overwritten document durably.
This requirement applies to each historical Graph node whose value you inspect. Event sourcing plus `DOCUMENT`
retains history; ordinary snapshot/cache settings do not prune Model events automatically.
...
For a worked before/after example and the distinction between root revisions and descendant changes, see
[Temporal Graphs](./175-temporal-graphs.mdx).
