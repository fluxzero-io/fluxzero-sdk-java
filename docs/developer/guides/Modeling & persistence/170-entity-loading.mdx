...
For document-backed Models, replayed direct documents remain in invisible staging. Register the complete application
Model catalog in the migration process and adopt the staged set after catch-up:

```java
Registration migrationModels = fluxzero.registerModelMigrationTypes(modelTypes);
int adopted = Fluxzero.adoptModelMigrations().join();
```
...
collection in the same transaction. The plural operation drains bounded batches and then rebuilds all materialized
Graph projections declared by the catalog; repeat the same call to resume an interrupted cutover. A value difference,
unknown staged type or concurrent legacy write retains staging and fails adoption.
