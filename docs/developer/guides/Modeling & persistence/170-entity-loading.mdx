...
### Backfilling event-sourced Models from legacy events
...
The handler must receive an indexed event that is still available in the co-located global event log. This primitive
currently supports event-sourced Models only. Legacy `STORE_ONLY` Aggregate events have no global index and cannot be
recovered through this route. Keep normal side-effect handlers out of the migration application and qualify document
adoption, catch-up, cutover and rollback separately before switching write ownership.
