...
In a dedicated migration application, track the original published events from the global event log and call
`Fluxzero.migratePublishedEvent()` while handling each applicable event:

```java
@HandleEvent
void migrate(LegacyOrderEvent event) {
...
`STORE_ONLY` Aggregate events have no global index and cannot be recovered through this route. Keep normal side-effect
handlers out of the migration application.

For document-backed Models, replayed direct documents remain in invisible staging. After catch-up, adopt each such
Model explicitly:

```java
Fluxzero.adoptModelMigration(orderId, Order.class).join();
```

Fluxzero deserializes and upcasts the staged value and any existing production document to the supplied Model type. It
then requires equal current values and unchanged staged state. If a production document exists, adoption also uses its
observed document index as a compare-and-set condition, leaves the document itself untouched, and atomically adds only
the Model write fence. If it does not exist, the staged document is copied into its normal collection in the same
transaction. A value difference or concurrent legacy write retains staging and fails adoption.

Qualify catch-up, an explicit final event boundary, handler cutover and rollback before switching write ownership.
...
`@Parent(path = "...")` edges. Pathless relationships remain available through typed `children(...)` and
