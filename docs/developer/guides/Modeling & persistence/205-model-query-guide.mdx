...
Changing stored fields? [Model migration tests](../model-migration-tests) connects value-preserving upcasts,
old-writer/new-reader reconstruction and queries before/after reindexing. A new read-time shape alone does not
change the server's stored query paths.
