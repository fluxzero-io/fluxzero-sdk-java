...
Use event handlers for business transitions that must each be processed. Document handling can reconcile pending
work only when the latest document retains all unfinished actions until they are acknowledged; see
[stateful handlers](../stateful-handlers/#choose-the-trigger-and-preserve-pending-work).
</Aside>
...
This ordinary return contract includes a Model's independent public DOCUMENT projection; it does not rewrite the
internal Model source. For guarded schema-only source rewriting use `@HandleDocument(modelState = Project.class)`
and return the upcast Project unchanged. A complete `Graph<Project>` returned from
`@HandleDocument(modelGraph = Project.class)` rewrites only the materialized Graph. These three routes update different
query paths; see [Testing Model migrations](../model-migration-tests). Ordinary projection writes do not acquire the
source handler's version/body compare-and-set guarantee.
...
- Current-state analytics (use events when every change must be counted or logged)
