...
This ordinary document-return contract is for application-owned documents/projections, not for rewriting
head-verified Model state outside its Model commit path. For Model schema evolution and the distinct complete-Graph
return contract, follow [Testing Model migrations](../model-migration-tests).
