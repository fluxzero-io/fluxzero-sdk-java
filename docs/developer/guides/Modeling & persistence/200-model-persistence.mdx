...
When an event-sourced aggregate is loaded, Fluxzero replays applied updates to rebuild current state. During this process, each event is re-applied
to the aggregate. See [Entity loading](/docs/guides/modeling-and-persistence/entity-loading/) for more information.
