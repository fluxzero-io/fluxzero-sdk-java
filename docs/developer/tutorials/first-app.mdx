...
Have your coding agent create a new project called 'Todo' using the Basic Starter for Java or Kotlin (see [Installation](/docs/getting-started/installation) for how).
...
Your coding agent can work in the project directly. Whenever you want to inspect or edit the code,
  <a href="https://www.jetbrains.com/idea/" target="_blank" rel="noopener noreferrer">IntelliJ IDEA <span aria-hidden="true">↗</span></a>
  provides a good Java and Kotlin experience.
...
<Aside type="note">
  ...
  <a href="https://jakarta.ee/specifications/bean-validation/3.1/" target="_blank" rel="noopener noreferrer">Jakarta Validation 3.1 ↗</a>.
  Annotations like <code>@NotNull</code> on commands and queries will be enforced automatically. The default validator
  can also inject the current message context, like <code>User</code> or <code>Metadata</code>, into constraint methods
  such as <code>@AssertTrue</code> when validation runs during message handling.
</Aside>
...
You can define any roles. For more info on user and role based access see [user access control](/docs/guides/messaging/role-based-access-control).
...
may have been published in the same app or any other service connected through Fluxzero.

You can inject all kinds of parameters into handler methods, like the command sender, metadata, or full command message. For more info on handlers see [message handlers](/docs/guides/messaging/message-handling).

<Aside type="note">
  Behind the scenes, messages like commands are consumed from the command log in Fluxzero by a consumer selected by
  your app's tracking defaults. For <code>fluxzero.defaults.version</code> values on or after
  <code>2026.05.20</code>, the default is a generated consumer per unconfigured handler; apps without that defaults
  marker, or with an older value, use the shared default command consumer.

  If you want more control, you can also select a mode with
  <code>fluxzero.tracking.unconfiguredHandlerConsumerMode</code> or create an isolated consumer with
  <code>@Consumer</code>.
  For details on consumer behavior, tuning, tracking positions, and the full versioned-defaults table, see
  [Message tracking](/docs/guides/messaging/message-tracking/) and
  [Application properties](/docs/guides/configuration/application-properties/).
</Aside>

<Aside type="tip">
  Handlers can be <a href="https://spring.io/projects/spring-framework" target="_blank" rel="noopener noreferrer">Spring <span aria-hidden="true">↗</span></a> components. This way they will get auto-registered with Fluxzero.
</Aside>
...
Want to understand why we recommend reusing the same payload in both command and event? See [Applying entity updates](/docs/guides/modeling-and-persistence/updating-entities#applying-updates-to-entities).
...
data class CreateProject(
    ...
    @Apply
    fun create(): Project = Project(projectId, details, null)
}
```
...
<Aside type="note">
  Recommended: avoid putting the sending user ID in command payloads. Use `Sender` injection for caller context.
</Aside>
...
<Aside type="tip">
  ...
  See [Entity persistence](/docs/guides/modeling-and-persistence/model-persistence).
</Aside>
...
<Aside type="note">
    ...
    which requires the fully qualified name:

    `"@class": "io.fluxzero.todo.project.command.CreateProject"`.
</Aside>
...
<Aside type="note">
  The default user in tests is the system user. For more info on how users get injected in production and in tests, see <a href="/docs/guides/messaging/071-role-based-access-control/#where-does-user-info-come-from">User Providers</a>
</Aside>
...
The GetProject query implements `Request<R>`, where `R` is the expected return type. Fluxzero validates this at
...
<Aside type='tip'>
This is the core idea: Fluxzero is built to let you focus on the simplest, most natural code. If you ever find a case where it doesn’t, [let us know](/contact) — that’s exactly the kind of feedback we love.
</Aside>
...
<Aside type="note">
  Java records and Kotlin data classes do not need a member-specific wither here. Fluxzero rebuilds the parent immutably when a task is added, replaced, or removed.
</Aside>
...
<Aside type="note">
  When loading an event-sourced <code>Entity</code> inside a handler method, Fluxzero automatically replays it up to
  the time of the handled message — not the present. This ensures you're always working with the entity state as it
  was when the message occurred. See [Entity loading](/docs/guides/modeling-and-persistence/entity-loading/) for more info.
</Aside>
...
- [Deploy your app to Fluxzero Cloud](/docs/tutorials/cloud-deployment)
