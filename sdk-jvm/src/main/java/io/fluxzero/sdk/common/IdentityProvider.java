 ...
 * If none are found, identifiers are generated by a lazily created {@link UuidFactory}.
 * </p>
 *
 * <h2>Example Usage</h2>
 * <pre>{@code
...
public interface IdentityProvider {
    ...
    /**
     * The default identity provider, resolved using {@link ServiceLoader}, or falling back to a lazily created
     * {@link UuidFactory}.
     */
    IdentityProvider defaultIdentityProvider = Optional.of(ServiceLoader.load(IdentityProvider.class))
            ...
            .map(Iterator::next)
            .orElseGet(IdentityProvider::newDefaultUuidProvider);

    private static IdentityProvider newDefaultUuidProvider() {
        return new IdentityProvider() {
            private volatile UuidFactory delegate;

            @Override
            public String nextFunctionalId() {
                return delegate().nextFunctionalId();
            }

            @Override
            public String idForName(String name) {
                return delegate().idForName(name);
            }

            @Override
            public String nextTechnicalId() {
                return delegate().nextTechnicalId();
            }

            private UuidFactory delegate() {
                UuidFactory value = delegate;
                if (value == null) {
                    synchronized (this) {
                        value = delegate;
                        if (value == null) {
                            delegate = value = new UuidFactory();
                        }
                    }
                }
                return value;
