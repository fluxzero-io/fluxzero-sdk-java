...
class CommandDeliveryTest {
        ...
        try (Fluxzero app = application(client, expected);
            ...
            other.apply(ignored -> {
                for (String namespace : new String[]{null, "other"}) {
                    app.documentStore().forNamespace(namespace).index("value", "id", "documents");
                    app.keyValueStore().forNamespace(namespace).store("key", "value");
                    app.keyValueStore().forNamespace(namespace).delete("key");
                    app.messageScheduler().forNamespace(namespace).schedule("schedule", Duration.ofHours(1));
            ...
            for (LocalClient transport : new LocalClient[]{client, namespaced}) {
                ...
                verify(transport.getKeyValueClient()).putValue(eq("key"), any(), eq(expected));
                verify(transport.getKeyValueClient()).deleteValue("key", expected);
                verify(transport.getSchedulingClient()).schedule(eq(expected), any(SerializedSchedule[].class));
    ...
    @Test
    void keyValueWritesAndSchedulingReturnBeforeAcknowledgement() {
        LocalClient client = client();
        try (Fluxzero app = application(client, Guarantee.STORED)) {
            CompletableFuture<Void> acknowledgement = new CompletableFuture<>();
            CompletableFuture<Boolean> conditional = new CompletableFuture<>();
            KeyValueClient keyValueClient = client.getKeyValueClient();
            SchedulingClient schedulingClient = client.getSchedulingClient();
            doReturn(acknowledgement).when(keyValueClient).putValue(anyString(), any(), any());
            doReturn(acknowledgement).when(keyValueClient).deleteValue(anyString(), any());
            doReturn(conditional).when(keyValueClient).putValueIfAbsent(anyString(), any());
            doReturn(acknowledgement).when(schedulingClient).schedule(any(), any(SerializedSchedule[].class));
            try {
                assertTimeoutPreemptively(Duration.ofSeconds(5), () -> {
                    assertFalse(app.keyValueStore().store("key", "value").isDone());
                    assertFalse(app.keyValueStore().delete("key").isDone());
                    assertFalse(app.keyValueStore().storeIfAbsent("key", "value").isDone());
                    app.messageScheduler().schedule("schedule", Duration.ofHours(1));
                    app.messageScheduler().scheduleCommand("command", Duration.ofHours(1));
                });
            } finally {
                acknowledgement.complete(null);
    ...
    void explicitDurabilityIsNotWeakenedByApplicationOverride() {
        ...
        try (Fluxzero app = application(client, Guarantee.NONE)) {
            ...
            app.keyValueStore().store("key", "value", Guarantee.STORED).join();
