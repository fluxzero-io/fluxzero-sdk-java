...
package io.fluxzero.javaclient.givenwhenthen.spring;
...
import io.fluxzero.javaclient.configuration.spring.ConditionalOnMissingProperty;
import io.fluxzero.javaclient.test.TestFixture;
import io.fluxzero.javaclient.test.spring.FluxzeroTestConfig;
...
class GivenWhenThenSpringTest {
    ...
    void createAndUpdateInSameBatch() {
        testFixture.whenExecuting(
                        fc -> Fluxzero.publishEvents(new StaticEvent("bla"), new StaticEvent("bla")))
                .expectCommands(1, 2);
    }
    ...
    record ConstructorHandler(@Association String someId, int eventCount) {
        ...
        ConstructorHandler(StaticEvent event) {
            ...
            Fluxzero.sendAndForgetCommand("constructor:" + eventCount);
        }

        @HandleEvent
        ConstructorHandler update(StaticEvent event) {
            Fluxzero.sendAndForgetCommand("constructor:" + (eventCount + 1));
            return toBuilder().eventCount(eventCount + 1).build();
        }
    }
