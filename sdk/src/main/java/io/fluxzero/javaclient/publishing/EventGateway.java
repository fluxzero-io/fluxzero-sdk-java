 ...
 * published to the Fluxzero Runtime for delivery to interested consumers.
 * <p>
 * Note that publishing an event via this gateway does <em>not</em> associate it with an aggregate. If you're
 * implementing an event-sourced aggregate, use {@link io.fluxzero.javaclient.modeling.Entity#apply} instead to
 ...
 * @see io.fluxzero.javaclient.modeling.Entity for applying events to aggregates
 * @see io.fluxzero.javaclient.tracking.handling.HandleEvent for handling events
 */
public interface EventGateway extends HasLocalHandlers {
    ...
    /**
     * Publishes the given {@link Message} to Fluxzero and/or local handlers. Returns a future that completes when
