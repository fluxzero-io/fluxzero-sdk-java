...
package io.fluxzero.javaclient.tracking.client;
...
import io.fluxzero.javaclient.tracking.BatchProcessingException;
import io.fluxzero.javaclient.tracking.ConsumerConfiguration;
import io.fluxzero.javaclient.tracking.FlowRegulator;
import io.fluxzero.javaclient.tracking.FluxzeroInterceptor;
import io.fluxzero.javaclient.tracking.Tracker;
import io.fluxzero.javaclient.tracking.TrackingException;
import io.fluxzero.javaclient.tracking.metrics.PauseTrackerEvent;
...
import static io.fluxzero.javaclient.tracking.BatchInterceptor.join;
 ...
 * <p>
 * Trackers with different names will receive the same messages. Trackers with the same name will not. (Fluxzero
 ...
 * batch size the tracker uses to fetch messages from Fluxzero. Each time the consumer has finished consuming a
 * batch the tracker will update its position with Fluxzero.
 * <p>
...
public class DefaultTracker implements Runnable, Registration {
     ...
     * <p>
     * {@link FluxzeroInterceptor} will be added to the list of batch interceptors in the given configuration. This
     * ensures that a thread local {@link Fluxzero} instance will always be available during tracking.
     * <p>
    ...
    public static Registration start(Consumer<List<SerializedMessage>> consumer, MessageType messageType,
        ...
        return start(consumer, messageType, null, config, fluxzero);
    }
