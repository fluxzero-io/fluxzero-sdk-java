...
import io.fluxzero.javaclient.tracking.handling.MutableHandler;
    ...
    public static class SocketEndpointWrapper {
        ...
        protected Object onOpenOrMessage(SocketSession session, DeserializingMessage message) {
            if (this.session == null) {
                ...
                Fluxzero.getOptionally().ifPresent(fc -> fc.beforeShutdown(() -> abort(1001)));
                trySchedulePing();
            }
            try {
        ...
        protected void trySchedulePing() {
            ofNullable(getPingDelay()).ifPresent(delay -> {
                if (isOpen()) {
                    var taskScheduler = Fluxzero.get().taskScheduler();
