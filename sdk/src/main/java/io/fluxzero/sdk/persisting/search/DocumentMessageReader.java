...
public final class DocumentMessageReader {
    ...
    /** Records accepted typed Graph methods until the returned registration is cancelled. */
    public synchronized Registration register(Object target, HandlerFilter filter) {
        ...
        if (topics.isEmpty()) { return Registration.noOp(); }
    ...
    public Stream<DeserializingMessage> read(List<SerializedMessage> messages, String topic, Serializer serializer) {
        ...
        return messages.stream().flatMap(message -> {
            ...
            if (!message.metadataContainsKey(ModelGraphDocumentManifest.METADATA_KEY)) {
                return decoded;
            }
            List<DeserializingMessage> roots = decoded.toList();
            if (roots.size() != 1) {
                throw new DeserializationException("Materialized Graph document %s requires exactly one root state; "
                        .formatted(message.getMessageId()) + "upcasting produced " + roots.size());
            }
