...
This also applies to ancestors created by earlier parts of the same commit, even when no Model history existed before it.
...
boundary; `getResult()` is null because no storage rejection occurred.
