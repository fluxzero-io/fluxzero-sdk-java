...
10. **Secure by Default**: Add `@RequiresUser` to your domain's `package-info.java` to protect all payloads within that
    package.
11. **Domain Errors**: Use **Error Interfaces** to group domain-specific exceptions.
...
public interface ProjectErrors {
    IllegalCommandException alreadyExists = new IllegalCommandException("Project already exists");
    UnauthorizedException unauthorized = new UnauthorizedException("Not authorized for this project");
}
```
