...
- For a composed independent-model graph returned as `JsonNode`, select its root with
  ...
  remain arrays whose items are complete model graphs. `type` and `modelGraph` are mutually exclusive. Runtime-served
  docs include registered child models from other modules.
