...
[//]: # (@formatter:off)
```kotlin
Fluxzero.publishMetrics(SearchLatencyMetric("projects", duration.toMillis()))
```
[//]: # (@formatter:on)
...
@HandleMetrics
fun on(event: IgnoreMessageEvent) {
    if (event.reason == IgnoreMessageEvent.EXPIRED_REQUEST) {
