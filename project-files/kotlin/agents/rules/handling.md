...
- For a composed independent-model graph returned as `JsonNode`, select its root with
  ...
  list of that child model and slash-separated prefixes become nested objects. `type` and `modelGraph` are mutually
  exclusive. Runtime-served docs include registered child models from other modules.
...
**Agent default:** return an ordinary payload or `Unit` and let known exceptions propagate. Do not add endpoint
