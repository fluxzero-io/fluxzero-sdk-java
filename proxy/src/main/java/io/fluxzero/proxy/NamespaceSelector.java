...
public class NamespaceSelector {
    ...
    /** Creates a namespace selector that validates token timestamps using the system clock. */
    public NamespaceSelector() {
        this(Clock.systemUTC());
    }

    NamespaceSelector(Clock clock) {
        jwtVerifier = memoize(() -> ApplicationProperties.mapProperty(
                JWKS_URL_PROPERTY, url -> new JwtVerifier(url, clock)));
    }
